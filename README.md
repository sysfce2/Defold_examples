...
* Use the `author_ids` array for one or more contributors. The legacy `author` display-name field is also accepted for a single contributor. Author IDs are stable lowercase kebab-case keys from the Defold website author registry; display names and profile metadata for ID-based authors do not belong in this repository.
